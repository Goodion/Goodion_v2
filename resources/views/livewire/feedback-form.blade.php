...
<x-base-container>
    ...
    <div x-data="{ feedbackForm: true }" @message-sent.window="feedbackForm = false" class="mx-auto w-1/2 bg-sky-dark border-2 rounded-t-xl border-sky-dark pl-5 py-3">
        <h2 x-show="feedbackForm" class="text-white text-xl">Будем рады получить от вас вопрос или отзыв!</h2>
        <h2 x-show="! feedbackForm" class="text-white text-xl">Спасибо за обращение.</h2>
    </div>
    <div class="mx-auto w-1/2 border-2 border-sky-dark py-10 px-16 rounded-b-xl">
        <form x-show="feedbackForm" class="pt-15" wire:submit.prevent="sendMessage">
            @guest
                <div class="mb-10">
                    <label class="font-medium" for="name">Представьтесь, пожалуйста<span class="text-red-500">*</span></label>
                ...
                <div class="mb-7 font-medium">
                    Оставьте <label for="email">E-mail</label> или <label for="phone">номер мобильного телефона</label><span class="text-red-500">*</span>
            ...
            @endguest
            <label class="font-medium" for="message">Ваш вопрос или комментарий</label><span class="text-red-500">*</span>
            <textarea wire:model.defer="feedback.message" class="@error('feedback.message') border-red-500 @endError focus:outline-none focus:ring-2 ring-forest border border-gray-300 rounded-lg py-2 px-4 block w-full appearance-none leading-normal" id="message" rows="4" name="message" placeholder="комментарий"></textarea>
